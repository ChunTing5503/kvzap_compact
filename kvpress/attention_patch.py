...
def attention_patch(func):
    ...
    def wrapper(module, query, key, value, attention_mask, dropout, **kwargs):
        query_decode = query.ndim >= 3 and query.shape[2] == 1

        # Some attention backends pass tensors with layouts that do not match the
        # legacy [bsz, heads, seq, dim] assumption used by the masking patch.
        # In that case, skip the fake-key path entirely and delegate directly.
        if query.ndim < 3 or key.ndim < 3:
            return time_region(
                "attention_decode_ms",
                query_decode,
                lambda: func(module, query, key, value, attention_mask, dropout, **kwargs),
            )
