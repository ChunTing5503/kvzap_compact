...
class KVzipPress(BasePress):
    ...
    _warned_about_prefill_overhead: bool = field(default=False, init=False, repr=False)
    ...
    def __call__(self, model: PreTrainedModel) -> Generator:
        ...
        if not self._warned_about_prefill_overhead:
            logger.warning(
            ...
            self._warned_about_prefill_overhead = True
    ...
    def compress_post(self, model: PreTrainedModel):
        """
        Convert KVzip scores into flattened per-head compaction.
        """
        if self.compression_ratio <= 0:
            return

        valid_masks = self._build_valid_masks()
        if not isinstance(self._cache, CompactedDynamicCache):
            raise RuntimeError(
                "KVzipPress requires the compacted KV-cache runtime. "
                "Use KVPressTextGenerationPipeline without overriding the cache."
            )
        self._cache.prune(valid_masks)

    def _build_valid_masks(self) -> torch.Tensor:
        n_layer, bsz, num_key_value_heads, ctx_len = self.score_val.shape
        valid_masks = torch.zeros_like(self.score_val, dtype=torch.bool)

        if self.layerwise:
            n_kept_per_layer = int((1 - self.compression_ratio) * bsz * num_key_value_heads * ctx_len)
            for layer_idx in range(n_layer):
                scores = self.score_val[layer_idx].reshape(bsz, -1)
                topk = scores.topk(n_kept_per_layer, dim=-1).indices
                valid = torch.zeros_like(scores, dtype=torch.bool)
                valid.scatter_(1, topk, True)
                valid_masks[layer_idx] = valid.view(bsz, num_key_value_heads, ctx_len)
            return valid_masks

        n_kept_total = int((1 - self.compression_ratio) * self.score_val.numel())
        flat_scores = self.score_val.reshape(-1)
        topk = flat_scores.topk(n_kept_total).indices
        valid_masks.view(-1)[topk] = True
        return valid_masks
