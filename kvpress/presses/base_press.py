...
class BasePress:
    ...
    def create_cache(self, model: PreTrainedModel):
        """
        Optional cache factory used by presses that require a custom cache type.
        """
        return None

    def after_prefill(self):
        """
        Optional hook invoked immediately after the prefill forward pass completes.
        """
        pass

    def snapshot_state(self):
        """
        Optional hook returning press-local state needed to restore context reuse.
        """
        return None

    def restore_state(self, snapshot):
        """
        Optional hook restoring state returned by ``snapshot_state``.
        """
        del snapshot

    def _get_cache_length(self, kwargs: dict, layer_idx: int) -> int:
        """
        Return the current KV-cache sequence length for a layer.

        Newer/alternate model implementations do not always forward `cache_position`
        into attention hooks, even though the cache has already been updated in-place.
        In that case we fall back to the populated cache length, which is the same
        quantity these hooks need after the forward pass.
        """
        ...
        if hasattr(cache_layer, "get_seq_length"):
            return int(cache_layer.get_seq_length())
        if hasattr(cache_layer, "keys") and cache_layer.keys is not None and cache_layer.keys.numel() > 0:
            return int(cache_layer.keys.shape[2])
        if hasattr(cache_layer, "values") and cache_layer.values is not None and cache_layer.values.numel() > 0:
            return int(cache_layer.values.shape[2])
        if hasattr(cache_layer, "cumulative_length"):
            return int(cache_layer.cumulative_length)
        raise KeyError("Could not infer cache length from attention hook kwargs or cache state.")

    def _is_prefill_phase(self, kwargs: dict, layer_idx: int) -> bool:
        hidden_states = kwargs["hidden_states"]
        q_len = hidden_states.shape[1]
        return self._get_cache_length(kwargs, layer_idx) <= q_len
    ...
    def __call__(self, model: PreTrainedModel) -> Generator:
        ...
        >>> press = DMSPress(press=KVzapPress(model_type="mlp"), threshold=-5.0)
        >>> with press(model):
